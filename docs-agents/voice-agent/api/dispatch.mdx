...
| `Idempotency-Key` | No | Free-form string. Repeating the same key returns the original `batch_id` instead of creating a new batch. See [Idempotency](#idempotency) below. |
